...
class cas_login
{
    ...
    {
        // Check for ticket of authentication
        ...
        } else if (!qa_opt('cas_login_allow_normal') || qa_request() != 'login'){
            // Forces the user to authenticate through a CAS Server
    ...
    // Layer to add HTML related to the login form in different contexts
    function login_html($tourl, $context)
    {
      echo('<a href="'.$this->cas->getServerLoginURL().'">'.qa_opt('cas_login_link_text').'</a>');
    }

    // Layer to add HTML related to the logout form
    ...
    private function cas_login_external_user($source, $identifier, $fields)
    /*
    ...
    */
    {
        ...
        } else if ($countusers) { // user exists so log them in
